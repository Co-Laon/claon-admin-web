...
import LeaveModal from '@/components/center/LeaveModal';
...
function Page() {
  ...
  const [leaveUrl, setLeaveUrl] = useState<string | undefined>();
  ...
  const handleRouterChangeStart = useCallback((url: string) => {
    if (!leaveUrl) {
      setLeaveUrl(url);
  ...
  const handleRouterChangeError = useCallback(() => {
  ...
  }, [mutateCenterProfileUploadList, mutateCenterImageUploadList, setValue]);
  ...
  return (
    <>
      {isLeaveModalOpen && (
        <LeaveModal
          onClickConfirm={handleLeaveModalConfirm}
          onClickCancel={handleLeaveModalCancel}
        />
      )}
...
Page.getLayout = function getLayout(page: ReactElement) {
  return (
    <RootLayout>
      <RegisterLayout step={50}>{page}</RegisterLayout>
    </RootLayout>
  );
};
